document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('.knwldge_hub_listing_sec').forEach(section => {
    ...
    function getCurrentOrder() {
      const sel = section.querySelector('input[name="date-sort-resources"]:checked');
      if (!sel) {
        return 'desc';
      }
      return (sel.id === 'Oldest-resources') ? 'asc' : 'desc';
    }
    ...
    const radioInputs = section.querySelectorAll('input[name="date-sort-resources"]');
    radioInputs.forEach(radio => {
      radio.addEventListener('change', () => {
        ...
        const order = (radio.id === 'Oldest-resources') ? 'asc' : 'desc';
        sortByDate(order);
      });
    });

    const defaultLatestRadio = section.querySelector('#Latest-resources');
    if (defaultLatestRadio) {
      if (!section.querySelector('input[name="date-sort-resources"]:checked')) {
        defaultLatestRadio.checked = true;
        const lab = defaultLatestRadio.closest('label');
        if (lab) lab.classList.add('w--redirected-checked');
      }
    }
    const loadMoreBtn = section.querySelector('.load_mre_bttn-resources');
    if (loadMoreBtn) {
